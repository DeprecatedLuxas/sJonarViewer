...
import {convertObjectToSearch, getParameterByName, parseSearchBar, verifyUser} from "../../utils";
...
class Home extends React.Component<any, HomePageState> {
    ...
    constructor(props: any) {
        ...
        this.state = {
            // @ts-ignore
            fromRedoSearch: (getParameterByName("fromRedoSearch", this.props.history.location.search) === "true"),
            // @ts-ignore
    ...
    componentDidMount() {
        // Verifying the users cookie.
        verifyUser(this.props);

        this.requestIsFromRedoSearch()
        // @ts-ignore
        this.props.history.push("/")
    }

    requestIsFromRedoSearch() {
        if (this.state.fromRedoSearch) {
            this.setState({
                search: convertObjectToSearch(JSON.parse(this.state.searchQuery))
            }, () => {
                this.retrieveData()
            });
        }
    ...
    retrieveData = async (event?: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await axios.get(`/api/search?query=${encodeURIComponent(JSON.stringify(parsedStateSearch))}`);
            this.setState({
